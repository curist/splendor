...
function playerStatus(player) {
  ...
  colors.forEach(color => {
    count += player.bonus[color] * 3 + player.resources[color];
  });
  return count;
}
...
function nextAvaCards(players, state, cards) {
  var tokencardlist = [];
  for (let i = 0; i < players.length; i++) {
    if (i != playerIndex) {
      let cardsthisplayer = getAffordableCards(players[i], cards);
      if (cardsthisplayer.length > 0){
        let bestcard = getBestCard(players[i], state, cardsthisplayer);
        if (bestcard) tokencardlist.push(bestcard);
      }
    }
  }
  return cards.filter(function(card) {
    const { key, provides } = card;
    var ifNotContain = true;
    for (let i = 0; i < tokencardlist.length; i++) {
      if(tokencardlist[i] && key == tokencardlist[i].key) {
        ifNotContain = false;
      }
    }
    return ifNotContain;
...
function cardValue(player, state, cards, card) {
  ...
  function valueForPlayer(player, card) {
    ...
    const playerstatus = playerStatus(player);
    ...
    var value = 0;
    if (playerstatus > 50) {
      value += card.points * 2 + totalNobleValue + cardBoardPoints(player, card) - cost;
    } else if (playerstatus > 45) {
      value += card.points * 1.5 + totalNobleValue + cardBoardPoints(player, card) - cost;
    } else {
      value += card.points + totalNobleValue + cardBoardPoints(player, card) - cost;
    }
    return value;
  }
...
export default class FiveYearsOld {
  ...
  turn (state) {
    ...
    // const allBonus = calcColorsTotal(allLeftCards);
    const resCount = countResources(player.resources);
    const playerstatus = playerStatus(player);
    if((resCount <= 7 && playerstatus <= 40) || (resCount <= 5 && playerstatus > 40)) {

      const bestLeftCards = nextAvaCards(state.players, state, bestCards);

      const topCards = bestLeftCards.slice(0,3);
